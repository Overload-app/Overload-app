...
describe("claudeChat", () => {
  ...
  // reviews) without needing a check duplicated at each call site. Real
  // second report, after a few days with this removed: a day's usage hit
  // $2 with no cap in place at all.
  ...
  test("does not block a paying subscriber or someone outside their trial, regardless of usage", async () => {
    vi.stubGlobal("navigator", { onLine: true });
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => ({ content: [{ type: "tool_use", name: "respond", input: { reply: "hi" } }] }) }));
  ...
  test("does not block a trial account still under budget, and records real usage from the response", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ content: [{ type: "tool_use", name: "respond", input: { reply: "hi" } }], usage: { input_tokens: 1000, output_tokens: 500 } }),
    }));
